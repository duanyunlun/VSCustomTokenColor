...
## 变更摘要
- 修复“扩展详情页 README 无法切换中英文”的展示策略：在 `README.md` 顶部增加语言锚点导航，并在同一 README 内提供 `中文/English` 两个分区（Marketplace/VS Code 目前只展示单个 README，无法按 UI 语言自动切换）。

## 变更原因
- 用户反馈 VS Code 扩展详情页 README 不能随语言切换；为避免体验割裂，改为单 README 双语分区。

## 验证方式
1) 本地打包：`npm run package`（确认 vsix 内的 `readme.md` 为双语分区版本）
